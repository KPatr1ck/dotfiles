...
-- reset indent highlights after :Telescope colorscheme
map('n', '<leader>rh', ':lua require("plugin-config.indent-blankline").set_indent_blankline_hl()<CR>', opt)

-- rest
map('n', '<leader>r', '<Plug>RestNvim', opt)
map('n', '<leader>rp', '<Plug>RestNvimPreview', opt)
...
pluginKeys.neoTreeList = {
    ...
    ['yp'] = 'copy', -- takes text input for destination, also accepts the optional config.show_path option like "add":
    -- ["c"] = {
    --  "copy",
    --  config = {
    --    show_path = "none" -- "none", "relative", "absolute"
